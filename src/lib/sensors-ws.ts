...
import { ErrorEnvelope, PagedEnvelope, } from './response-envelopes.js';
...
async function addData<T>(url: URL, data: Record<string, string>,
  ...
  try {
    ...
    // // If the response is ok, convert the result using the displayFn
    const displayResult = displayFn(result.result);
    return Errors.okResult(displayResult);
  } catch (error) {
...
async function findData<T>(url: URL,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return Errors.errResult(errorEnvelope);
    }

    // If the response is ok, convert the result using the displayFn
    const result: PagedEnvelope<T> = await response.json();
    const displayValues = result.result.map(r => displayFn(r.result));
    const displayResult: PagedValues = {
      values: displayValues,
      next: result.links.next?.href,
      prev: result.links.prev?.href,
    };
